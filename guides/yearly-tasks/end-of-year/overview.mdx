...
- **You're in the final month of your financial year (June)** - The system restricts EOY setup to June only
- All transactions for the current year have been reconciled
- You're ready to start planning for the next financial year

<Note>
**Important:** The EOY process can only be started during June (the final month of the fiscal year). If you try to access it in other months, the system will display a restriction message and block the process.
</Note>

## Process Summary

The EOY process consists of **three main phases** with clear menu-driven progression:

### 1. **Initial Setup** (in Master Spreadsheet)
...
### 2. **Configuration & Setup** (in New Copy)
...
### 3. **Finalization** (in New Copy)
   ...
   - **Automatically archive the previous master** with "\_ARCHIVED_FY{year}" suffix
...
<Warning>
While the system checks some requirements automatically, it's your responsibility to ensure all accounts are properly reconciled before starting the EOY process.
</Warning>
...
4. Follow the three-step process through to finalization
