...
When you finalize the EOY process in the new copy, the system will automatically:

1. Mark the new spreadsheet as the master
2. Rename the original master spreadsheet to include "\_ARCHIVED_FY{year}"
3. Add a hidden "\_Archive" sheet with metadata to the original spreadsheet

This archiving process helps maintain a clear distinction between your active budget and historical records while preserving all your previous data.
...
- The archived master will still have its script properties intact, so avoid using it for active budgeting
