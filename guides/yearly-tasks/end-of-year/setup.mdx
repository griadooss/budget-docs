...
description: 'Detailed guide for the End of Year budget setup process'
---

# EOY Budget Setup

This guide provides comprehensive instructions for the End of Year (EOY) budget process. You can refer to this documentation at any point during the process for detailed guidance.

## Overview of the Process

The EOY process follows these main steps:
...
5. **Finalize EOY Setup**
   ...
   - Automatically archive the legacy master
...
4. Verifies the TimeZone settings
...
### 3. New Year Budget Setup

1. In the confirmation dialog, click **YES** to proceed
2. The system will import settings from the original spreadsheet
   - Note: The IS_MASTER flag will initially be imported as TRUE but should be FALSE until the EOY process is complete
3. In the New Year Budget Setup dialog:
...
After configuring settings and clicking Save, the system will:

1. **Automatically display the "Budget Setup Instructions" dialog** with guidance on updating your budget
2. Activate the Maintain Budget sheet
3. Scroll to the income section to help you begin the update process

> **Note:** If you close the instructions dialog and need to see it again, you can find the same information in this documentation.

### 1. Review and Update Income Sources
...
2. For each income source:
   ...
   - The checkbox in Column I indicates whether the income has been distributed to the Annual Budget sheet

> **Note:** Currently, when the Frequency (F) or Budget (E) values are changed for income items, the distribute flag is not automatically reset.

### 2. Review and Update Expense Categories
...
2. For each expense category:
   ...
   - Update budget amounts in Column E
   - Adjust frequencies in Column F if needed (12 for monthly, 1 for annual)
   - The checkbox in Column I indicates whether the expense has been distributed to the Annual Budget sheet
   - When you update the frequency or budget amount, this flag will automatically reset
...
4. All budget items have their distribution checkboxes properly set in Column I
...
2. The system will:
   ...
   - Mark this spreadsheet as the new master (setting IS_MASTER to TRUE)
   - Automatically archive the legacy master spreadsheet (rename and mark as archived)
   - Show a completion message

### 4. Legacy Master Archiving

When you finalize the EOY setup, the system will automatically archive your previous master spreadsheet by:

1. Renaming it to include "\_ARCHIVED_FY{year}" suffix (e.g., "Budget_MASTER_ARCHIVED_FY2024")
2. Adding a hidden "\_Archive" sheet with metadata including:
   - The date of archiving
   - A marker indicating it was the former master
   - The ID of the new master spreadsheet

> **Important Note:** Due to Google Apps Script limitations, the archived spreadsheet will still have its script properties set as if it were the master (IS_MASTER = true). This is a technical limitation that cannot be resolved without complex workarounds. If you open the archived spreadsheet, it may still show the "MASTER" indicator and have full functionality, but you should not use it for active budgeting.
...
### Archived Master Issues

1. If the original master is not properly archived (renamed):

   - You can manually rename it to include "\_ARCHIVED_FY{year}"
   - This is purely cosmetic and won't affect functionality

2. If you accidentally open and use the archived spreadsheet:
   - You should switch back to the new master spreadsheet
   - The archived spreadsheet should be considered read-only
